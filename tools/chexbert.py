...
class CheXbert(nn.Module):
    def __init__(self, ckpt_dir, bert_path, checkpoint_path, device, p=0.1):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(bert_path, cache_dir='/working/rajan/multiview-llm/.cache')
        # config = BertConfig().from_pretrained(os.path.join(ckpt_dir, bert_path), local_files_only=True) 
        config = BertConfig().from_pretrained(bert_path, local_files_only=True) #RR
        ...
        with torch.no_grad():
            ...
            state_dict = torch.load(os.path.join(checkpoint_path), map_location=device)['model_state_dict']
    ...
    def forward(self, reports):
        ...
        with torch.no_grad():
            ...
            tokenized = self.tokenizer(reports, padding='longest', truncation=True, max_length=512,return_tensors="pt")
