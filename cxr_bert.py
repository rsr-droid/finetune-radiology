...
class CXRBERT(Metric):
    ...
    def compute(self, epoch):
        ...
        if self.accumulate_over_dicoms:
            # Ensure 'study_id' is not a list before grouping
            cxrbert['study_id'] = cxrbert['study_id'].apply(lambda x: x[0] if isinstance(x, list) else x)
            cxrbert = cxrbert.drop(['dicom_id'], axis=1).groupby('study_id', as_index=False).mean()

        if isinstance(cxrbert, dict):
            return cxrbert
        else:
            # If the output is not a dict, wrap it in a dict
            return {'cxr_bert_metric': cxrbert.similarity.mean()}
